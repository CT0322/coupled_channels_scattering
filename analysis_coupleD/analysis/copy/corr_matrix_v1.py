...
__all__ = ["create_corr_matrix_cosh", "create_corr_matrix_sinh", "calculate_gevp", "shift_corr_matrix", "write_eigenvals_cosh", "write_eigenvals_sinh", "reorder_by_evec", "reorder_by_eval"]
...
# sort t0+1 and t0-1 by eigen values
       ...
       order[t0] = np.array(range(dim))
       if t0-1 >= 0:
         order[t0-1] = np.array(list(sorted(range(dim), key = eigen_values[_n, t0-1].__getitem__)))

       for _t in range(t0+2, T):
         for _i2 in range(dim):
             Ct0ev2 = np.dot(Ct0[_n], eigen_vectors[_n, _t, :, _i2])
             ev1Ct0ev2 = np.abs(np.dot(eigen_vectors[_n, _t-1, :, range(dim)], Ct0ev2[0]))
#             ev1Ct0ev2_check = np.abs(np.dot(eigen_vectors[_n, _t, :, range(dim)], Ct0ev2[0]))
    ...
    for _n in range(nsamples):
       ...
       for _t in range(t0+2, T):
         ...
         for _perm in itertools.permutations(range(dim)):
            evec = eigen_vector(_n, t0+1)
            evec_product = np.identity(dim)
            for _i in range(dim):
              evec(:, _i) = eigen_vector(_n, _t, :, _perm[_i])
