...
def reorder_by_eval(eigen_values, eigen_vectors, t0):
    ...
    order_eigen_values = np.zeros_like(eigen_values)
    order_eigen_vectors = np.zeros_like(eigen_vectors)
    for _n in range(nsamples):
       for _t in range(T):
          ...
          for _i in range(dim):
             order_eigen_values[_n, _t, order[_t,_i]] = eigen_values[_n, _t, _i]
             order_eigen_vectors[_n, _t, :, order[_t,_i]] = eigen_vectors[_n, _t, :, _i]
    return order_eigen_values, order_eigen_vectors 
    ...
    for _n in range(nsamples):
       ...
       for _t in range(T):
         ...
         for _perm in itertools.permutations(range(dim)):
            ...
            for _i in range(dim):
              evec_temp = eigen_vectors[_n, t0+1].copy()
              evec_temp[:, _perm[_i]] = eigen_vectors[_n, _t, :, _i]
              evec_product = np.dot(evec_product, evec_temp)
            det = np.abs(np.linalg.det(evec_product))
            if det > det_max:
               det_max = det
               order[_t] = order[t0+1][np.array(_perm)]
...
#         print("*********%i************" % _t)
         ...
         for _perm in itertools.permutations(range(dim)):
            evec_product = np.identity(dim)
            for _i in range(dim):
              evec_temp = eigen_vectors_norm[_n, t0+1].copy()
              evec_temp[:, _perm[_i]] = eigen_vectors_norm[_n, _t, :, _i]
              evec_product = np.dot(evec_product, evec_temp)
            det = np.abs(np.linalg.det(evec_product))
#            print(_perm)
#            print(det)
...
def solve_gevp_gen(a, t_0, sym=True):
    ...
    # initialization
    count = 0
    # calculate the eigensystem for t in (0, T2)
    for j in range(0, T2):
        try:
