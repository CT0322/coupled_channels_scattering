...
__all__ = ["bootstrap", "boot_corr", "sym_and_boot", "sym"]
...
def bootstrap(source, nbsamples):
    ...
    boot = np.zeros(nbsamples, dtype=complex)
...
def boot_corr(source, nbsamples = 1000):
    """boostraps correlation functions.
    ...
    if _T > 1:
       for t in range(1, _T):
          boot = np.c_[boot, bootstrap(source[:,t], nbsamples)]
    return boot
