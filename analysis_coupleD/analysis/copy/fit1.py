...
def fitting(fitfunc, X, Y, start_parm, correlated=True, verbose=False):
    ...
    try:
        ...
        y = np.mean(Y, axis=0)
        p,cov1,infodict,mesg,ier = leastsq(errfunc, start_parm, args=(X, y, cov), full_output=1)
...
def genfit(data, tmin, tmax, min_t_interval, fitfunc, start_params, out_originfit, out_bootstrapfit, out_finalresults, verbose=False):
...
#    print(fit_intervals)
    ...
    for _i in range(ninter):
            ...
            if fitting(fitfunc, tlist[lo:up+1], data[:,lo:up+1], start_params, verbose=False) == 0:
               ...
               _res, _res_cov, _chi2, _pval = fitting(fitfunc, tlist[lo:up+1], data[:,lo:up+1], start_params, verbose=False)
               res.append(_res)
...
#    print(pval.shape)
    ...
    for _i in range(ninter):
        for _j in range(npar):
           weight[_i, _j] = (1.-2.*np.fabs(pval[_i] - 0.5))*np.amax(res_error[:,_j])/res_error[_i,_j]
    ...
    np.savetxt(out_finalresults, out_final, fmt=["%.10f", "%.10f", "%.10f", "%.10f", "%.10f"])
